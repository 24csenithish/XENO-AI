...
class ResponseValidator:
    def __init__(self):
        # Tuple of (compiled pattern, replacement text)
        self.identity_leaks = [
    ...
    def validate(self, text: str) -> str:
        """Validate local model response, sanitize identity leaks, and formatting checks."""
        if not text or not text.strip():
            logger.warning("Local model returned empty output during validation")
            return "XENO local AI engine returned an empty response. Please adjust your request or configure another active model."
